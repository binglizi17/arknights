...
class SimEnvironment:
    ...
    def _process_robot_control(self, step_size):
        """
        处理机器人控制：只对机械臂 6 DoF 做路径规划，腿部与底盘保持不动。
        """
        # 1. 获取末端命令
        with data_lock:
        ...
        if ee_new and not self._tracking_arm:
            ...
            try:
                ...
                q_arm_goal = np.array(ik_action.joint_positions, dtype=np.float64)  # shape (6,)
                q_arm_start = q_full[arm_indices].copy()

                # 2.3 尝试动态获取真实关节限位
                view = self.go2.robot._articulation_view
                if hasattr(view, "get_limits"):
                    lower, upper = view.get_limits()            # 期望返回 (array[D], array[D])
                ...
                all_limits = np.stack([lower, upper], axis=1)  # shape (D,2)
                joint_limits6 = all_limits[arm_indices].tolist()  # 6×2
                ...
                # 2.6 拼回全关节轨迹
                M = traj6.shape[0]
                traj_full = np.tile(q_full, (M, 1))      # (M, D)
                traj_full[:, arm_indices] = traj6        # 仅替换臂部关节
