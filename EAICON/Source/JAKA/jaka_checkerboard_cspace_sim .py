    ...
    def step(self):
        if self.kit.is_running():
            ...
            if self.ee_pose_pub:
                ...
                print(f'publish: {position}, {euler}')
                ee_pose_msg = self.create_pose_stamped_msg(position,euler,'map')
                self.ee_pose_pub.publish(ee_pose_msg)
            #发布关节信息
            if self.jointstate_pub:
                arm_jointstate = self.robotArmController.get_joints_state()
                jointstate_msg = self.create_joint_state(name=self.robotArmController._articulation.dof_names,position=arm_jointstate.positions,velocity=arm_jointstate.velocities,effort=np.zeros(8))
